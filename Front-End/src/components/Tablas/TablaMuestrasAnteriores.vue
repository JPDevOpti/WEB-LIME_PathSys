          ...
          <thead>
            <tr class="border-b border-gray-200">
              <th 
                ...
                :class="column.class"
              >
                <div v-if="column.key === 'selection'" class="flex items-center justify-center">
                  <input
                    type="checkbox"
                    :checked="allSelected"
            ...
            >
              <td class="px-5 py-4 sm:px-6">
                <div class="flex items-center justify-center">
                  <input
                    type="checkbox"
                    :checked="selectedMuestras.has(muestra.id)"
                    @change="toggleSelectMuestra(muestra.id)"
                    class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500"
                  />
                </div>
              </td>
          ...
          >
            ...
            <div class="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 rounded-b-2xl">
              <div class="flex justify-end gap-3">
                ...
                <button
                  @click="descargarMuestra(muestraSeleccionada)"
...
const selectedMuestras = ref<Set<string>>(new Set())
...
const availableMonths = computed(() => {
  ...
  const availableMonthNumbers = muestras.value
    .filter(muestra => new Date(muestra.fechaRecepcion).getFullYear() === parseInt(selectedYear.value))
...
const filteredMuestras = computed(() => {
  ...
  if (selectedYear.value) {
    filtered = filtered.filter(muestra => {
      return new Date(muestra.fechaRecepcion).getFullYear() === parseInt(selectedYear.value)
    })
  }
  ...
  // Ordenar
  filtered.sort((a, b) => {
    let aValue: string | Date = a[sortKey.value as keyof Muestra]
    let bValue: string | Date = b[sortKey.value as keyof Muestra]
...
// Watch para limpiar selecciones cuando cambian los filtros
watch([searchQuery, selectedYear, selectedMonth, selectedAnalisis, selectedEstado], () => {
  currentPage.value = 1
  selectedMuestras.value.clear()
})
...
// Computed property para verificar si todas las muestras están seleccionadas
const allSelected = computed(() => {
  return paginatedMuestras.value.length > 0 && 
         paginatedMuestras.value.every(muestra => selectedMuestras.value.has(muestra.id))
})

// Computed property para verificar si algunas muestras están seleccionadas
const someSelected = computed(() => {
  return paginatedMuestras.value.some(muestra => selectedMuestras.value.has(muestra.id))
})

// Función para seleccionar/deseleccionar todas las muestras
const toggleSelectAll = () => {
  if (allSelected.value) {
    paginatedMuestras.value.forEach(muestra => selectedMuestras.value.delete(muestra.id))
  } else {
    paginatedMuestras.value.forEach(muestra => selectedMuestras.value.add(muestra.id))
  }
}

// Función para seleccionar/deseleccionar una muestra
const toggleSelectMuestra = (muestraId: string) => {
  if (selectedMuestras.value.has(muestraId)) {
    selectedMuestras.value.delete(muestraId)
  } else {
    selectedMuestras.value.add(muestraId)
  }
}

// Función para descargar la muestra
const descargarMuestra = (muestra: Muestra) => {
  // Aquí puedes implementar la lógica de descarga
  console.log('Descargando muestra:', muestra.id)
  // Por ejemplo, podrías hacer una llamada a la API para obtener el PDF
  // y luego usar window.open o crear un enlace de descarga
  muestraSeleccionada.value = null
}
</script>
